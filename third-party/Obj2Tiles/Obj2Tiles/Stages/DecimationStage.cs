...
{
    public static async Task<DecimateResult> Decimate(
        string sourcePath,
        string destPath,
        int lods,
        double? minGeometryQuality = null)
    {
        var qualities = BuildLodQualities(lods, minGeometryQuality);
    ...
    internal static float[] BuildLodQualities(int lods, double? minGeometryQuality)
    {
        if (lods <= 1)
            return [];

        if (minGeometryQuality == null)
        {
            return Enumerable.Range(0, lods - 1)
                .Select(i => 1.0f - ((i + 1) / (float)lods))
                .ToArray();
        }

        var minimum = (float)minGeometryQuality.Value;
        return Enumerable.Range(0, lods - 1)
            .Select(i => 1.0f - ((i + 1) / (float)(lods - 1)) * (1.0f - minimum))
            .ToArray();
    }
