    ...
    internal class Program
    {
        ...
        private static async Task Run(Options opts)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine(" => Spatial-first, absolute-error HLOD pipeline");
                    destFolderSplit = createTempFolder($"{pipelineId}-obj2tiles-hlod");

                    var hierarchy = await StagesFacade.BuildHierarchical(
                        opts.Input,
                        destFolderSplit,
                        opts.LODs,
                        opts.SplitPointStrategy,
                        opts.HlodErrorDivisor,
                        opts.HlodTargetRatio,
                    ...
                    GltfConverterOptions? hierarchicalGltfOptions = null;
                    if (opts.TextureFormat == TextureFormat.Ktx2)
                    {
                        hierarchicalGltfOptions = new GltfConverterOptions
                        {
                            EncodeKtx2 = true,
                    ...
                    Console.WriteLine($" => Tiling HLOD with measured root error {hierarchy.RootGeometricError:0.######}");
                    StagesFacade.Tile(
                        destFolderSplit,
                        tilesetOutput,
                        hierarchy.LodCount,
                        hierarchy.RootGeometricError,
                        hierarchy.Bounds,
                        hierarchicalGps,
                        opts.LocalMode,
                        isOctree: true,
                        rootSourceObj: null,
                        gltfOptions: hierarchicalGltfOptions,
                        geometricErrors: hierarchy.GeometricErrors,
                        externalTilesetDepth: opts.ExternalTilesetDepth,
                        forceReplaceRoot: true);
                ...
                // A configurable geometry floor prevents the coarsest LOD from collapsing
                // regular structures such as roads, roof lines and building outlines.
                var decimateRes = await StagesFacade.Decimate(
                    opts.Input, destFolderDecimation, opts.LODs, opts.MinGeometryQuality);
        ...
        private static bool CheckOptions(Options opts)
        {
            ...
            if (opts.MinGeometryQuality is <= 0 or > 1)
            {
                Console.WriteLine(" !> --min-geometry-quality must be in the (0, 1] range");
                return false;
            }

            if (opts.Hierarchical && opts.StopAt != Stage.Tiling)
            {
                Console.WriteLine(" !> --hierarchical requires the full Tiling stage");
                return false;
            }

            if (opts.Hierarchical && opts.ZSplit)
            {
                Console.WriteLine(" !> --hierarchical currently builds the recommended XY quadtree; do not combine it with --zsplit");
                return false;
            }

            if (opts.HlodErrorDivisor <= 0)
            {
                Console.WriteLine(" !> --hlod-error-divisor must be greater than zero");
                return false;
            }

            if (opts.HlodTargetRatio is <= 0 or > 1)
            {
                Console.WriteLine(" !> --hlod-target-ratio must be in the (0, 1] range");
                return false;
            }

            if (opts.ExternalTilesetDepth < 0)
            {
                Console.WriteLine(" !> --external-tileset-depth must be non-negative");
                return false;
            }
